...
import kafka.storm.DynamicPartitionConnections;
import kafka.storm.GlobalPartitionId;
import kafka.storm.HostPort;
import kafka.storm.KafkaConfig.StaticHosts;
import kafka.storm.KafkaConfig.ZkHosts;
...
public class KafkaUtils {
	...
	public static List<GlobalPartitionId> getOrderedPartitions(
			Map<String, List> partitions) {
	...
	public static Map emitPartitionBatchNew(TridentKafkaConfig config,
			SimpleConsumer consumer, GlobalPartitionId partition,
			TridentCollector collector, Map lastMeta,
			String topologyInstanceId, String topologyName,
		...
		if (lastMeta != null) {
			...
			if (config.forceFromStart
				...
				offset = consumer.getOffsetsBefore(config.topic,
						partition.partition, config.startOffsetTime, 1)[0];
			} else {
			...
			offset = consumer.getOffsetsBefore(config.topic,
					partition.partition, startTime, 1)[0];
		}
		ByteBufferMessageSet msgs;
		try {
			long start = System.nanoTime();
			msgs = consumer.fetch(new FetchRequest(config.topic,
					partition.partition, offset, config.fetchSizeBytes));
		...
		newMeta.put("topology",
				ImmutableMap.of("name", topologyName, "id", topologyInstanceId));
		return newMeta;
	}

	public static void emit(TridentKafkaConfig config,
			TridentCollector collector, Message msg) {
		Iterable<List<Object>> values = config.scheme.deserialize(Utils
	...
	public static class KafkaOffsetMetric implements IMetric {
		...
		public void setLatestEmittedOffset(GlobalPartitionId partition,
				long offset) {
		...
		public Object getValueAndReset() {
			try {
				...
				if (_partitions != null
					...
					for (Map.Entry<GlobalPartitionId, Long> e : _partitionToOffset
							.entrySet()) {
						...
						long latestTimeOffset = consumer.getOffsetsBefore(
								_topic, partition.partition,
		...
		public void refreshPartitions(Set<GlobalPartitionId> partitions) {
			...
			Iterator<GlobalPartitionId> it = _partitionToOffset.keySet()
					.iterator();
			while (it.hasNext()) {
